...
public class Draw extends ReceiverAdapter implements ActionListener, ChannelListener {
	...
	public void viewAccepted(View v) {
		...
		if (v instanceof MergeView) {
			...
			// This is an example of a simple merge function, which fetches the state from
			// the coordinator
	...
	protected class DrawPanel extends JPanel implements MouseMotionListener {
		...
		/**
		 * Adds pixel to queue and calls repaint() whenever we have MAX_ITEMS pixels in
		 * the queue or when MAX_TIME msecs have elapsed (whichever comes first). The
		 * advantage compared to just calling repaint() after adding a pixel to the
		 * queue is that repaint() can most often draw multiple points at the same time.
		 */
